# House Shell Generator for SketchUp
# Section: House Shell
...
lBackWidthIn = 180
hLeftBarWidthIn = 120
hRightBarWidthIn = 120
hMiddleBarHeightIn = 120
hMiddleBarOffsetIn = 180
tTopWidthIn = 360
tTopLengthIn = 120
tStemWidthIn = 120
tStemLengthIn = 240
...
u_w8 = 480
u_direction = 'back'
l_direction = 'front-left'
wall_height_in = 96
wall_thickness_in = 6
foundation_type = 'stem-wall'
slab_thickness = 4
thickened_edge_depth = 12
stem_wall_height = 30
stem_wall_above = 18
stem_wall_below = 12
stem_wall_thickness = 8
footing_width = 16
footing_thickness = 8
...
# --- FLOOR OPTIONS ---
add_floor_framing = true
joist_spacing = 16
joist_size = '2x10'
joist_direction = 'y'
add_subfloor = true
subfloor_thickness = 0.75
rim_joist_thickness = 1.5
enable_girder_system = false
girder_span_threshold = 12
girder_post_spacing = 8
girder_size = '3-2x10'
girder_post_size = '6x6'
girder_pier_size = '12" Round'
add_pocket_beams = true
pocket_beams_only_at_girder_ends = false
floor_bays = [
...
upper_floor_wall_height = 96
upper_floor_joist_size = '2x10'
subfloor_material = 'osb'

# --- LAYERS ---
add_sheathing = true
sheathing_thickness = 0.5
add_insulation = true
insulation_thickness = 3.5
add_drywall = true
...
# --- ANGLED EXTERIOR WALLS (traced diagonals) ---
angled_walls = [

]

# --- TRACED SHELL FOOTPRINT LOOPS (per floor, model inches) ---
shell_outlines = [

]
shell_outline_for = -> (fi) {
  o = shell_outlines.find { |s| s[:floor_index] == fi } || shell_outlines.find { |s| s[:floor_index] == 0 }
  o ? o[:pts] : []
}
...
plate_height = 1.5
door_ro_allowance = 2
window_ro_allowance = 0

# --- DIMENSIONS ---
generate_dimensions = true
solid_walls_only = false
...
# --- WALL FRAMING SPECIALISTS ---
      ...
      draw_finishes = -> (w_ents, params, openings, sh_ext, dw_ext, is_interior, w_height, z_off) {
        ...
        # Hole centers are the rough-opening midpoints relative to the WALL start;
        # each layer's panel may start before the wall (extensions), so shift the
        # hole by that layer's start extension to keep it wall-aligned.
        ro_holes = (openings[:doors] + openings[:windows]).map { |op| { c: (op[:os] + op[:oe])/2.0, w: op[:w], h: op[:h], sill: op[:sill] || 0 } }
        layer_ops = -> (ext0) { ro_holes.map { |o| { local_ox: o[:c] + ext0, w: o[:w], h: o[:h], sill: o[:sill] } } }
        ...
        if add_sheathing && !is_interior
          ...
          draw_wall_layer.call(w_ents, sh_x, sh_y, z_off, is_x ? length + sh_ext[0] + sh_ext[1] : sheathing_thickness,
                              is_x ? sheathing_thickness : length + sh_ext[0] + sh_ext[1], w_height, is_x, layer_ops.call(sh_ext[0]), "Sheathing", mat)
        end
        ...
        if add_insulation
          ...
          draw_wall_layer.call(w_ents, in_x, in_y, z_off, is_x ? length : insulation_thickness,
                              is_x ? insulation_thickness : length, w_height, is_x, layer_ops.call(0), "Insulation", mat)
        end

        if add_drywall
          mat = get_material.call("Drywall", color_drywall)
          if is_interior
            # interior walls get drywall on BOTH faces
            if is_x
              draw_wall_layer.call(w_ents, sx, sy - drywall_thickness, z_off, length, drywall_thickness, w_height, is_x, layer_ops.call(0), "Drywall", mat)
              draw_wall_layer.call(w_ents, sx, sy + depth, z_off, length, drywall_thickness, w_height, is_x, layer_ops.call(0), "Drywall", mat)
            else
              draw_wall_layer.call(w_ents, sx - drywall_thickness, sy, z_off, drywall_thickness, length, w_height, is_x, layer_ops.call(0), "Drywall", mat)
              draw_wall_layer.call(w_ents, sx + depth, sy, z_off, drywall_thickness, length, w_height, is_x, layer_ops.call(0), "Drywall", mat)
            end
          else
            ...
            draw_wall_layer.call(w_ents, dw_x, dw_y, z_off, is_x ? length + dw_ext[0] + dw_ext[1] : drywall_thickness,
                                is_x ? drywall_thickness : length + dw_ext[0] + dw_ext[1], w_height, is_x, layer_ops.call(dw_ext[0]), "Drywall", mat)
          end
        end
      }
      ...
      draw_wall_framing = -> (wall_id, start_x, start_y, length, depth, is_x_dir, ext_dir, sh_s=0, sh_e=0, dw_s=0, dw_e=0, is_int=false, add_corners=false, target_ents = f_ents, w_height = wall_height_in, z_off = 0, floor_idx = 0) {
        ...
        # x_in is the opening's LEFT edge along the wall (2D preview convention);
        # the rough opening is centered on the unit itself
        ...
        openings = {
          doors: doors.select{|d| d[:wall] == wall_id && (d[:floor_index] || 0) == floor_idx}.map{|d|
            ...
            center = d[:x_in] + d[:width_in]/2.0
            { ox: d[:x_in], os: center - w_total/2.0, oe: center + w_total/2.0, w: w_total, h: d[:height_in] + door_ro_allowance }
          },
          windows: windows.select{|w| w[:wall] == wall_id && (w[:floor_index] || 0) == floor_idx}.map{|w|
            ...
            center = w[:x_in] + w[:width_in]/2.0
            { ox: w[:x_in], os: center - w_total/2.0, oe: center + w_total/2.0, w: w_total, h: w[:height_in] + window_ro_allowance, sill: w[:sill_height_in] }
          }
        }
      ...
      # Angled (traced diagonal) walls: frame them as a plain horizontal wall
      # inside a local group, then rotate/translate that group into place.
      # The centerline runs A->B; the local body (y 0..t) is shifted -t/2 so
      # it straddles the centerline exactly like the 2D preview. Dimensions
      # are suppressed because they draw in world coordinates.
      draw_angled_wall = -> (aw, target_ents = f_ents, w_height = wall_height_in, z_off = 0, floor_idx = 0) {
        aw_len = Math.sqrt((aw[:bx]-aw[:ax])**2 + (aw[:by]-aw[:ay])**2)
        next if aw_len < 1
        theta = Math.atan2(aw[:by]-aw[:ay], aw[:bx]-aw[:ax])
        parent = target_ents.add_group
        parent.name = "Angled Wall #{aw[:id]}"
        prev_dims = generate_dimensions
        generate_dimensions = false
        begin
          draw_wall_framing.call(aw[:id], 0, 0, aw_len, aw[:thickness_in], true, aw[:ext_dir], 0, 0, 0, 0, false, false, parent.entities, w_height, z_off, floor_idx)
        ensure
          generate_dimensions = prev_dims
        end
        parent.transform!(
          Geom::Transformation.translation(Geom::Vector3d.new(aw[:ax], aw[:ay], 0)) *
          Geom::Transformation.rotation(Geom::Point3d.new(0,0,0), Geom::Vector3d.new(0,0,1), theta) *
          Geom::Transformation.translation(Geom::Vector3d.new(0, -aw[:thickness_in]/2.0, 0))
        )
      }
  joist_h = 9.25
  ...
  # Scanline spans of a plan polygon: interior intervals along the line where
  # the given axis-coordinate equals v ('x' -> y-ranges, 'y' -> x-ranges).
  poly_spans = -> (poly_pts, v, axis) {
    hits = []
    poly_pts.each_with_index do |a, i|
      b = poly_pts[(i + 1) % poly_pts.length]
      a1 = axis == 'x' ? a.x : a.y
      b1 = axis == 'x' ? b.x : b.y
      a2 = axis == 'x' ? a.y : a.x
      b2 = axis == 'x' ? b.y : b.x
      if (a1 <= v && b1 > v) || (b1 <= v && a1 > v)
        hits << a2 + (v - a1) / (b1 - a1) * (b2 - a2)
      end
    end
    hits.sort!
    spans = []
  ...
  draw_floor_framing = -> (pts_raw, target_ents = f_ents, z_off = 0, j_size = joist_size) {
    ...
    if solid_walls_only
      ...
      # Traced shells: rims follow every footprint edge (including the 45s)
      # and joists are clipped to the polygon interior, so nothing pokes
      # past a clipped corner.
      area2 = 0.0
      ...
      pts.each_with_index do |p1, i|
        ...
        next if elen < 0.5
        dirv = vec.normalize
        n = left_in ? Geom::Vector3d.new(-dirv.y, dirv.x, 0) : Geom::Vector3d.new(dirv.y, -dirv.x, 0)
        rg = j_group.entities.add_group
        rg.name = "Rim Joist"
        r_pts = [
          [p1.x, p1.y, j_z], [p2.x, p2.y, j_z],
          [p2.x + n.x * rt, p2.y + n.y * rt, j_z], [p1.x + n.x * rt, p1.y + n.y * rt, j_z]
        ]
        rf = rg.entities.add_face(r_pts)
        if rf
          rf.reverse! if rf.normal.z < 0
          rf.pushpull(curr_joist_h)
        end
      end
      if joist_direction == 'y'
        ...
        num_joists.times do |i|
          ...
          poly_spans.call(pts, jx + t / 2.0, 'x').each do |s0, s1|
            seg = s1 - s0 - 2 * rt
            draw_box.call(j_group.entities, jx, s0 + rt, j_z, t, seg, curr_joist_h, "Floor Joist") if seg > 1
          end
        end
      else
        num_joists = (local_l / joist_spacing).ceil + 1
        num_joists.times do |i|
          jy = min_y + i * joist_spacing
          jy = max_y - t if jy + t > max_y
          poly_spans.call(pts, jy + t / 2.0, 'y').each do |s0, s1|
            seg = s1 - s0 - 2 * rt
            draw_box.call(j_group.entities, s0 + rt, jy, j_z, seg, t, curr_joist_h, "Floor Joist") if seg > 1
          end
        end
      end
  ...
  # 1. Foundation
  if ['all', 'foundation'].include?("all") && foundation_type != 'none'
    ...
    if foundation_shape == 'rectangle'
      ...
      outline0 = shell_outline_for.call(0)
      if outline0.length >= 3
        # Traced shell: one foundation from the real footprint loop — the
        # polygon path handles slab, thickened edges and stem walls per edge
        f_pts = outline0.map { |p| [p[0], p[1], 0] }
        draw_foundation.call(f_pts)
      elsif combined_blocks.length > 0
  ...
  # 2. Floor Framing
  if ['all', 'floor'].include?("all") && add_floor_framing
    ...
    if shape == 'rectangle'
      ...
      outline0 = shell_outline_for.call(0)
      if outline0.length >= 3
        # Traced shell: floor system follows the footprint loop
        f_pts = outline0.map { |p| [p[0], p[1], 0] }
        draw_floor_framing.call(f_pts)
      elsif combined_blocks.length > 0
        combined_blocks.each do |block|
  ...
  # 3. Exterior Walls
  if ['all', 'exterior'].include?("all") || "all".to_i > 0
    ...
    angled_walls.select{|aw| (aw[:floor_index] || 0) == 0}.each do |aw|
      draw_angled_wall.call(aw)
    end
  end

  # 4. Interior Walls
  if ['all', 'interior'].include?("all") || "all".to_i > 0
  ...
  if ['all', 'exterior', 'interior', 'floor'].include?("all") && additional_stories > 0
    ...
    additional_stories.times do |story_i|
      ...
      if ['all', 'floor'].include?("all") && add_floor_framing
        ...
        if shape == 'rectangle'
          ...
          uo = shell_outline_for.call(floor_num)
          if uo.length >= 3
            f_pts = uo.map { |p| [p[0], p[1], wall_z] }
            draw_floor_framing.call(f_pts, f_ents, wall_z, upper_floor_joist_size)
          end
        end
      end
      ...
      if ['all', 'exterior'].include?("all")
        ...
        angled_walls.select{|aw| (aw[:floor_index] || 0) == floor_num}.each do |aw|
          draw_angled_wall.call(aw, f_ents, upper_floor_wall_height, wall_z, floor_num)
        end
      end
      
      # Upper story interior walls
      if ['all', 'interior'].include?("all")
      ...
      if run[:spacing_in] > 0
        ...
        if run[:type] == 'Solid Shell'
          ...
          if run[:custom_corners]
            ...
            # Hip roof: 4 trapezoidal/triangular faces — respects per-edge overrides
            ow = w + 2 * overhang
            od = d + 2 * overhang
            ox = rx - overhang
            oz = rz - overhang
            ...
            # Edge overrides
            eo = run[:edge_overrides] || {}
            resolve_edge = ->(edge_val) {
              val = edge_val || 'auto'
              val == 'auto' ? 'hip' : val
            }
            ...
            begin
              if run[:rotation] == 0
                ...
                edge_w = resolve_edge.call(eo[:west])
                edge_e = resolve_edge.call(eo[:east])
                
                ridge_z = oz + od * ratio
                # Ridge endpoints: extend to wall if gable/wall, inset if hip/valley
                rl_x = (edge_w == 'hip' || edge_w == 'valley') ? ox + hip_inset : ox
                rr_x = (edge_e == 'hip' || edge_e == 'valley') ? ox + ow - hip_inset : ox + ow
                ridge_l = [rl_x, ridge_z, ridge_y]
                ridge_r = [rr_x, ridge_z, ridge_y]
                ...
                # Left (west) end
                if edge_w == 'hip' || edge_w == 'valley'
                  g.entities.add_face(nw, ridge_l, sw) rescue nil
                elsif edge_w == 'gable'
                  # Vertical gable wall triangle at the wall face, not overhang edge
                  gw_nw = Geom::Point3d.new(rx, rz, y)
                  gw_sw = Geom::Point3d.new(rx, rz + d, y)
                  gw_peak = Geom::Point3d.new(rx, rz + d * ratio, ridge_y)
                  g.entities.add_face(gw_nw, gw_peak, gw_sw) rescue nil
                end
                # 'wall': no face at this end
                
                # Right (east) end
                if edge_e == 'hip' || edge_e == 'valley'
                  g.entities.add_face(ne, se, ridge_r) rescue nil
                elsif edge_e == 'gable'
                  ge_ne = Geom::Point3d.new(rx + w, rz, y)
                  ge_se = Geom::Point3d.new(rx + w, rz + d, y)
                  ge_peak = Geom::Point3d.new(rx + w, rz + d * ratio, ridge_y)
                  g.entities.add_face(ge_ne, ge_se, ge_peak) rescue nil
                end
                ...
                edge_n = resolve_edge.call(eo[:north])
                edge_s = resolve_edge.call(eo[:south])
                
                ridge_x = ox + ow * ratio
                rl_y = (edge_n == 'hip' || edge_n == 'valley') ? oz + hip_inset : oz
                rr_y = (edge_s == 'hip' || edge_s == 'valley') ? oz + od - hip_inset : oz + od
                ridge_l = [ridge_x, rl_y, ridge_y]
                ridge_r = [ridge_x, rr_y, ridge_y]
                ...
                # Front (north) end
                if edge_n == 'hip' || edge_n == 'valley'
                  g.entities.add_face(nw, ridge_l, ne) rescue nil
                elsif edge_n == 'gable'
                  gn_nw = Geom::Point3d.new(rx, rz, y)
                  gn_ne = Geom::Point3d.new(rx + w, rz, y)
                  gn_peak = Geom::Point3d.new(rx + w * ratio, rz, ridge_y)
                  g.entities.add_face(gn_nw, gn_peak, gn_ne) rescue nil
                end
                
                # Back (south) end
                if edge_s == 'hip' || edge_s == 'valley'
                  g.entities.add_face(sw, se, ridge_r) rescue nil
                elsif edge_s == 'gable'
                  gs_sw = Geom::Point3d.new(rx, rz + d, y)
                  gs_se = Geom::Point3d.new(rx + w, rz + d, y)
                  gs_peak = Geom::Point3d.new(rx + w * ratio, rz + d, ridge_y)
                  g.entities.add_face(gs_sw, gs_se, gs_peak) rescue nil
                end
  ...
  puts "Generation of House Shell Completed Successfully!"
rescue => e
  model.abort_operation
  puts "Error generating House Shell: #{e.message}"
